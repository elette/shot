...
//   if (target != null) $E(target).innerHTML = '<img src="/shot/images/loading.gif"/>';
  ...
  if (window.XMLHttpRequest) {
    ...
    req.onreadystatechange = function() {XHRDone(target);};
...
function drawCanvas() {
    ...
    var strList='';
    for (i=0; i<arrShot.length; i++)
        strList += arrShot[i][1].title.text + "<a href='javascript:removeShot(" + i +");'>¡¿</a><br>";
...
//                         arrShot[arrShot.length-1][2] = setInterval(function() {
//                                     y2 = eval("[" + CHR(url) + "]");
//                                     var len = series.data.length;
//                                     shift = len > 10-1;
// //                                     series.addPoint([(new Date()).getTime(), Math.random()], true,true);
//                                     series.addPoint(y2, false,shift);
//
